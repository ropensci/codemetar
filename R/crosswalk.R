...
#' @examples
#' crosswalk("Zenodo")
crosswalk <- function(column, cw_table = NULL, to_json = FALSE){
  ...
  df <- stats::na.omit(cw_table[c("Parent Type", "Property", "Type", column)])
  ...
  context
  if(to_json){
    jsonlite::toJSON(list(context = context), pretty = TRUE, auto_unbox = TRUE)
  } else {
    context
  }
}
